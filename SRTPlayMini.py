...
STYLE_SUFFIX = (
    ...
    "cartoon drawing style, elderly characters, minimalistic, no text overlay." 
)
# ─── session_state ─────────────────────────────
...
def gerar_prompt(client_txt, texto: str) -> str:
    pedido = (
        ...
        #"This biblical scene, that bring emotion and a beautiful look, bright and sharpness, few bright spots, set against a beautiful ancient Middle Eastern backdrop. Face emotion when its important. The prompt should end with the quality parameters."
        "This scene, that bring emotion. The prompt should end with the quality parameters."
        f"\n\nScene:\n{texto}\n\nQuality parameters:\n{STYLE_SUFFIX}"
    )
    try:
...
def gerar_imagem_replicate(prompt: str, aspect_ratio: str="1:1") -> bytes:
    output = replicate.run("black-forest-labs/flux-schnell", input={"prompt": prompt, "aspect_ratio": "1:1", "output_format": "png", "output_quality": 100, "disable_safety_checker": True, "go_fast": False}) 
